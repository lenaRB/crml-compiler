...
model CountInside_externals
  ...
  CRML.ETL.Connectors.TimeLocatorOutput tl1
    annotation (Placement(transformation(origin = {20, 0}, extent = {{100, -40}, {120, -20}}), iconTransformation(extent = {{100, -40}, {120, -20}})));
  ...
  CRML.ETL.Connectors.ClockOutput   clock1
    annotation (Placement(transformation(origin = {20, 0}, extent = {{100, 0}, {120, 20}}), iconTransformation(extent = {{100, 0}, {120, 20}})));
  ...
  CRML.ETL.Connectors.CRMLPeriodOutput P1
    annotation (Placement(transformation(origin = {20, 0}, extent = {{100, -100}, {120, -80}}), iconTransformation(extent = {{100, -100}, {120, -80}})));
  CRML.ETL.Connectors.CRMLClockOutput C1 annotation(
    Placement(transformation(origin = {130, 50}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, 50}, extent = {{-20, -20}, {20, 20}})));
  CRML.ETL.Types.CRMLClockClock cRMLClockClock annotation(
...
equation
  ...
  connect(cRMLPeriodTimePeriod.y, P1)
    annotation (Line(points={{61,-90},{130,-90}}, color={255,170,255}));
  connect(cRMLPeriodTimePeriod.tl, during.y[1])
    annotation (Line(points={{50,-80},{50,-70}}, color={0,0,255}));
  connect(clockEvent.y, cRMLClockClock.c) annotation(
    Line(points = {{42, 10}, {48, 10}, {48, 40}, {59, 40}}, color = {175, 175, 175}));
  connect(cRMLClockClock.y, C1) annotation(
    Line(points = {{82, 40}, {88, 40}, {88, 50}, {130, 50}}, color = {175, 175, 175}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio = false, extent = {{-60, 80}, {120, -100}})));
