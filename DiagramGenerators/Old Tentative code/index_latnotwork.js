...
async function main() {
  ...
  classes.forEach(c => {
    ...
    if (sub.length && sup.length) {
      arrowEdges.push({ source: `table_${sub[0].name}`, target: `table_${sup[0].name}`, type: 'inheritance', label: '' });
    }
  });
  // Contains edges
  containsRelations.forEach(r => {
    ...
    if (owner.length && contained.length) {
      arrowEdges.push({ source: `table_${owner[0].name}`, target: `table_${contained[0].name}`, type: 'contains', label: 'contains' });
    }
  });
  // Uses edges
  usesRelations.forEach(r => {
    ...
    if (owner.length && used.length) {
      arrowEdges.push({ source: `table_${owner[0].name}`, target: `table_${used[0].name}`, type: 'uses', label: 'uses' });
    }
  });
  ...
  objects.forEach((obj, idx) => {
    ...
    xml += `      <mxCell id="${tableId}" style="shape=table;startSize=20;container=1;recursiveResize=0;collapsible=0;tableLayout=fixed;html=1;overflow=visible;strokeColor=none;"` +
           ` vertex="1" parent="1">\n`;
  ...
  arrowEdges.forEach((e, i) => {
    ...
    const label = e.label ? e.label.replace(/&/g, '&').replace(/"/g, '"').replace(/</g, '<').replace(/>/g, '>') : '';
    xml += `      <mxCell id="edge${i}" value="${label}" style="${style}" edge="1" source="${e.source}" target="${e.target}" parent="1">\n`;
