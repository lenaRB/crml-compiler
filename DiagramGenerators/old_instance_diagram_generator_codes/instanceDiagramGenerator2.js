...
async function main() {
  ...
  classes.forEach((cls, i) => {
    ...
    const y = 60 + row * (hHdr + hAtt * cls.attributes.length + hMth * cls.methods.length + gapY);
    const cid = nextId(); pos[cls.name] = cid;
    const totalH = hHdr + cls.attributes.length * hAtt + cls.methods.length * hMth;
    xml += `      <mxCell id="${cid}" style="umlClass;html=1;rounded=0;strokeColor=#000;" vertex="1" parent="1">\n`;
    xml += `        <mxGeometry x="${x}" y="${y}" width="${wCls}" height="${totalH}" as="geometry"/>\n`;
    xml += '      </mxCell>\n';
    // Header
    const hid = nextId();
    const headerLabel = escapeXml(`<b>${cls.name}</b>`);
    xml += `      <mxCell id="${hid}" value="${headerLabel}" style="text;html=1;verticalAlign=middle;align=center;" vertex="1" parent="${cid}">\n`;
    ...
    cls.attributes.forEach((a, j) => {
      ...
      xml += `      <mxCell id="${aid}" value="${attrLabel}" style="text;html=1;verticalAlign=middle;align=left;" vertex="1" parent="${cid}">\n`;
    ...
    cls.methods.forEach((m, k) => {
      ...
      xml += `      <mxCell id="${mid}" value"${methLabel}" style="text;html=1;verticalAlign=middle;align=left;" vertex="1" parent="${cid}">\n`;
  ...
  instances.forEach((inst, i) => {
    ...
    const x = 60 + col * (wCls + gapX);
    const y = 80 + colsCls * (hHdr + gapY) + row * (hHdr + 20);
    const iid = nextId(); pos[inst.name] = iid;
    const instLabel = escapeXml(`<i>${inst.name}: ${inst.type}</i>`);
    xml += `      <mxCell id="${iid}" value="${instLabel}" style="shape=ellipse;html=1;rounded=1;strokeColor=#333;fillColor=#F0F0F0;" vertex="1" parent="1">\n`;
  ...
  associations.forEach(rel => {
    ...
    const style = rel.type === 'association'
      ...
      : 'edgeStyle=orthogonalEdgeStyle;endArrow=open;dashed=1;';
  ...
  const outFile = inputFile.replace(/\.crml$/, '') + '_class_object.drawio';
